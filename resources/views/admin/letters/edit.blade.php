<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('letters.update', $letter->id) }}" method="POST" class="bg-white overflow-hidden shadow-2xl sm:rounded-[3rem] p-10 relative z-10 border-t-[10px] border-blue-500" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="flex gap-4 mb-10 border-b-2 border-gray-50 pb-8">
                    <div class="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-3xl shrink-0 border border-blue-100">📋</div>
                    <div>
                        <h3 class="text-2xl font-black text-sipega-navy leading-none italic uppercase -mt-1">Metadata Dokumen</h3>
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 leading-relaxed">Formulir edit ST/SK terintegrasi dengan mesin Anti-Clash & Performa Pegawai (Tukin).</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="group">
                            ...
                            <input type="text" name="number" value="{{ old('number', $letter->number) }}" placeholder="123/C6.24/KP.10.00/2026" class="w-full border-2 border-gray-100 rounded-2xl p-4 text-sm font-bold bg-gray-50 focus:bg-white focus:border-sipega-navy focus:ring-0 transition-all">
                        </div>
                        ...
                        <div class="group">
                            ...
                            <textarea name="title" required rows="3" placeholder="Contoh: Rapat Koordinasi Wilayah..." class="w-full border-2 border-gray-100 rounded-2xl p-4 text-sm font-bold bg-gray-50 focus:bg-white focus:border-sipega-navy focus:ring-0 transition-all resize-none">{{ old('title', $letter->title) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="group">
                                ...
                                <input type="date" name="date_start" value="{{ old('date_start', $letter->date_start) }}" class="w-full border-2 border-gray-100 rounded-xl p-4 text-xs font-black text-sipega-navy bg-blue-50/30 focus:border-sipega-navy focus:ring-0 transition-all">
                            </div>
                            <div class="group">
                                <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-2 ml-1">Tgl Selesai</label>
                                <input type="date" name="date_end" value="{{ old('date_end', $letter->date_end) }}" class="w-full border-2 border-gray-100 rounded-xl p-4 text-xs font-black text-sipega-navy bg-blue-50/30 focus:border-sipega-navy focus:ring-0 transition-all">
                            </div>
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-2 ml-1">Lokasi Dinas/Penugasan</label>
                            <input type="text" name="location" value="{{ old('location', $letter->location) }}" placeholder="Ex: Hotel Aston, Jakarta" class="w-full border-2 border-gray-100 rounded-2xl p-4 text-sm font-bold bg-gray-50 focus:bg-white focus:border-sipega-navy focus:ring-0 transition-all">
                        </div>
                        ...
                        <div class="group">
                            ...
                            <textarea name="justification" rows="2" placeholder="Tuliskan justifikasi/alasan override jadwal di sini..." class="w-full border-2 border-red-100 bg-red-50/30 rounded-2xl p-4 text-xs font-medium focus:bg-white focus:border-red-400 focus:ring-0 transition-all resize-none placeholder-red-300">{{ old('justification', $letter->justification) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 bg-gray-50 rounded-3xl p-8 border-2 border-gray-100 relative">
                    ...
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-6">Sistem akan mengecek jadwal bentrok & performa secara otomatis saat diupdate.</p>

                    <div class="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                        @php
                            $selectedIds = $letter->users->pluck('id')->toArray();
                            $oldUsers = is_array(old('users')) ? old('users') : $selectedIds;
                        @endphp
                        ...
                        <label class="flex items-center justify-between p-4 bg-white rounded-2xl border-2 border-transparent hover:border-sipega-orange cursor-pointer transition-all shadow-sm group has-[:checked]:border-sipega-orange has-[:checked]:bg-orange-50/20">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" name="users[]" value="{{ $user->id }}" class="w-5 h-5 text-sipega-orange bg-gray-100 border-gray-300 rounded focus:ring-sipega-orange focus:ring-2 cursor-pointer" {{ in_array($user->id, $oldUsers) ? 'checked' : '' }}>
                                </div>
