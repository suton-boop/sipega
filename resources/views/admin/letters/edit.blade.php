        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <h2 class="font-black text-2xl text-sipega-navy leading-tight uppercase tracking-wider">
                    ✏️ Edit Surat Tugas: {{ $letter->number ?? 'DRAFT' }}
                </h2>
            </div>
            @if($letter->status === 'Approved')
            <span class="px-4 py-2 bg-emerald-100 text-emerald-800 text-xs font-black rounded-2xl border border-emerald-200 flex items-center gap-1.5">
                <span class="w-2 h-2 bg-emerald-500 rounded-full"></span> SUDAH DI-APPROVE
            </span>
            @endif
        </div>
    </x-slot>

    <div class="py-8" x-data="suratTugasEditApp()">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form action="{{ route('letters.update', $letter->id) }}" method="POST" id="stEditForm">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 shadow-xl space-y-6">
                    <div>
                        ...
                        <h3 class="text-xl font-black text-sipega-navy mt-2">Model Surat Tugas</h3>
                    </div>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 shadow-xl space-y-4 mt-6">
                    <div>
                        ...
                        <h3 class="text-xl font-black text-sipega-navy mt-2">Kategori Surat Tugas</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <label class="flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all"
                            ...
                            <div>
                                ...
                                <span class="text-[11px] font-bold text-gray-600">Kegiatan Kantor</span>
                            </div>
                        </label>
                        ...
                        <label class="flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all"
                            ...
                            <div>
                                ...
                                <span class="text-[11px] font-bold text-gray-600">Kegiatan Pusat</span>
                            </div>
                        </label>
                        ...
                        <label class="flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all"
                            ...
                            <div>
                                ...
                                <span class="text-[11px] font-bold text-gray-600">Kegiatan Kemitraan</span>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 shadow-xl space-y-6 mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Nomor Surat Tugas <span class="text-red-500">*</span></label>
                            <input type="text" name="number" value="{{ old('number', $letter->number) }}" required class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>

                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Nama Kegiatan / Perihal <span class="text-red-500">*</span></label>
                            <input type="text" name="title" value="{{ old('title', $letter->title) }}" required class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Instansi Pengundang / Pejabat</label>
                            <input type="text" name="invitation_from" value="{{ old('invitation_from', $letter->invitation_from) }}" class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>

                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Nomor Surat Undangan</label>
                            <input type="text" name="invitation_number" value="{{ old('invitation_number', $letter->invitation_number) }}" class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>
                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Tanggal Surat Undangan</label>
                            <input type="date" name="invitation_date" value="{{ old('invitation_date', $letter->invitation_date ? $letter->invitation_date->format('Y-m-d') : '') }}" class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Perihal pada Undangan (Hal)</label>
                        ...
                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Tanggal Selesai <span class="text-red-500">*</span></label>
                            <input type="date" 
                                   name="date_end" 
                                   x-model="dateEnd" 
                                   @change="fetchConflicts()"
                        ...
                        <div class="md:col-span-2 pt-2 border-t border-gray-100">
                            <label class="inline-flex items-center gap-3 p-3 bg-amber-50/60 rounded-2xl border border-amber-200 cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <span class="text-[11px] font-medium text-amber-800">Menampilkan kolom Keterangan (seperti SPMI, LITNUM, DIGITALISASI) di tabel personil dan PDF.</span>
                                </div>
                            </label>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-50 text-[10px] font-black uppercase tracking-wider text-gray-500 border-b border-gray-200">
                                    ...
                                    <th class="py-3 px-4 min-w-[180px]">NIP & Golongan</th>
                                    ...
                                    <template x-if="selectedModel === 'model_4'">
                                        <th class="py-3 px-4 min-w-[120px]">Peran</th>
                                    </template>
                                    <template x-if="selectedModel === 'model_4'">
                            ...
                            <tbody class="divide-y divide-gray-100 text-xs">
                                <template x-for="(row, index) in participants" :key="index">
                                    <tr :class="busyUsers[row.user_id] ? 'bg-red-50/70 border-2 border-red-300' : 'hover:bg-gray-50/50'">
                                        ...
                                        <template x-if="showKeterangan || selectedModel === 'model_5'">
                                            <td class="py-3 px-4">
                                                <input type="text" :name="`participants[${index}][keterangan]`" x-model="row.keterangan" placeholder="SPMI / LITNUM..." class="w-full text-xs font-bold rounded-xl border-gray-200 py-2">
                                            </td>
                                        </template>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 shadow-xl space-y-6 mt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="signatory_name" value="{{ old('signatory_name', $letter->signatory_name ?? 'Dr. Jarwoko, M. Pd') }}" class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>
                        <div>
                            <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">NIP Pejabat</label>
                            <input type="text" name="signatory_nip" value="{{ old('signatory_nip', $letter->signatory_nip ?? '197003191997031001') }}" class="w-full text-xs font-bold rounded-2xl border-gray-200 focus:border-sipega-orange focus:ring-sipega-orange px-4 py-3">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                            <label :class="signatureDateType === 'auto' ? 'border-sipega-orange bg-orange-50/40 text-orange-950' : 'border-gray-200 bg-white text-gray-700'" class="flex items-start gap-3 p-3.5 rounded-2xl border-2 cursor-pointer transition">
                                ...
                                <div>
                                    ...
                                    <span class="text-[11px] text-gray-500 font-medium">Otomatis mengikuti tanggal dibuatnya surat: <b class="text-sipega-orange">{{ $letter->created_at ? $letter->created_at->translatedFormat('d F Y') : \Carbon\Carbon::now()->translatedFormat('d F Y') }}</b></span>
                                </div>
                            </label>
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <label class="flex items-start gap-3 p-4 bg-orange-50/70 hover:bg-orange-50 rounded-2xl border border-orange-200 cursor-pointer transition">
                            <input type="checkbox" name="break_closing_paragraph" value="1" {{ old('break_closing_paragraph', $letter->break_closing_paragraph) ? 'checked' : '' }} class="mt-1 rounded border-gray-300 text-sipega-orange focus:ring-sipega-orange w-4 h-4">
                            <div>
                    ...
                    <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row justify-end items-center gap-4">
                        <input type="hidden" name="action_type" id="editActionType" value="save">
                        ...
                        <button type="submit" onclick="document.getElementById('editActionType').value='save'" class="px-6 py-3 bg-sipega-navy hover:bg-black text-white font-black text-xs uppercase tracking-widest rounded-2xl transition shadow-lg flex items-center gap-2">
    ...
    <script>
        ...
        function suratTugasEditApp() {
            return {
                ...
                signatureDateType: '{{ old('signature_date_type', $letter->signature_date_type ?? 'auto') }}',
                signedAt: '{{ old('signed_at', $letter->signed_at ? $letter->signed_at->format('Y-m-d') : ($letter->created_at ? $letter->created_at->format('Y-m-d') : date('Y-m-d'))) }}',
                busyUsers: {},
